...
import '../domain/track_detail_event.dart';
import '../domain/track_detail_state.dart';
...
class TrackDetailBloc extends Bloc<TrackDetailEvent, TrackDetailState> {
  ...
  Future<void> _onFetch(
    ...
    try {
      ...
      if (track == null) {
        emit(state.copyWith(status: TrackDetailStatus.failure, error: 'Track not found'));
      } else {
        emit(state.copyWith(status: TrackDetailStatus.success, track: track));
      }
    } catch (e) {
      emit(state.copyWith(status: TrackDetailStatus.failure, error: e.toString()));
    }
  }
}
