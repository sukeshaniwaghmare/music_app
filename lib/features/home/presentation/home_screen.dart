...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadAll() async {
    print('DATABASE_LOG: HomeScreen._loadAll started');
    setState(() => _loading = true);
    try {
      print('DATABASE_LOG: HomeScreen calling fetchSongs for old_song and english');
      final results = await Future.wait([
        _supabase.fetchSongs(limit: 20, category: 'old_song'),
        _supabase.fetchSongs(limit: 20, category: 'english'),
      ]);
      print('DATABASE_LOG: HomeScreen fetchSongs finished. Results: ${results[0].length} and ${results[1].length}');
      if (mounted) {
        setState(() {
          _oldSongs = results[0];
  ...
  Future<void> _logout() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to logout and lock premium features?', 
    ...
    if (confirm == true) {
      ...
      if (mounted) {
        SystemNavigator.pop(); // Simplest way to "logout" in this setup is to close or reset
      }
    }
  }
