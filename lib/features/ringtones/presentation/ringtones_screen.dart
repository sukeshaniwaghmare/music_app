  ...
  Future<void> _fetch(String lang) async {
    ...
    try {
      await Future.wait(queries.map((q) async {
        final uri = Uri.parse('https://itunes.apple.com/search?term=${Uri.encodeComponent(q)}&entity=song&limit=30&country=IN');
  ...
  Future<void> _downloadRingtone() async {
    ...
    try {
      ...
      if (mounted && Platform.isAndroid) {
        ...
        if (await file.exists()) _showSetRingtoneDialog(filePath, fileName);
      } else if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Saved: $fileName'), behavior: SnackBarBehavior.floating,
  ...
  void _showSetRingtoneDialog(String filePath, String fileName) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Later', style: TextStyle(color: Colors.white38))),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1565C0), foregroundColor: Colors.white),
            onPressed: () { Navigator.pop(context); _openRingtoneSettings(); },
        ...
        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          _downloading
              ...
              : IconButton(
                  ...
                  onPressed: _downloadRingtone, padding: EdgeInsets.zero,
                ),
        ]),
      ),
    );
