...
class SupabaseService {
  ...
  static Future<void> initialize() async {
    print('DATABASE_LOG: New Supabase initializing...');
    await Supabase.initialize(url: _url, anonKey: _anonKey);
    print('DATABASE_LOG: New Supabase initialized.');
  }
  ...
  Future<List<Track>> fetchSongs({int offset = 0, int limit = 50, String? category}) async {
    ...
    print('DATABASE_SONG_DATA (fetchSongs): $response');
  ...
  Future<List<Track>> searchSongs(String query) async {
    final response = await client
        ...
        .limit(100);
    print('DATABASE_SONG_DATA (searchSongs): $response');
    return (response as List).map((e) => Track.fromSupabase(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<Track>> fetchSongsFromFolder(String folderName) async {
    ...
    print('DATABASE_SONG_DATA (folder response 1): $response');
    ...
    print('DATABASE_SONG_DATA (folder response 2): $response2');
